...
async function testTauriInitialization() {
  ...
  try {
    ...
    const { waitForTauriInitialization, debugTauriContext, testTauriConnection } = window.tauriDebugUtils || {};
    
    if (!waitForTauriInitialization) {
      console.error("❌ Test utilities not available - run this in the actual app context");
      return;
    }
