...
export const waitForTauriInitialization = async (
  ...
  while (Date.now() - startTime < maxWaitMs) {
    ...
    const isInTauriProcess =
      typeof window !== "undefined" && (window as any).__TAURI_METADATA__;
    const hasInvokeFunction = typeof invoke === "function";

    console.log(
      `⏱️ ${
        Date.now() - startTime
      }ms - Tauri global: ${!!hasTauriGlobal}, Metadata: ${!!isInTauriProcess}, Invoke: ${hasInvokeFunction}`
    );
    ...
    if (hasInvokeFunction) {
      try {
        ...
        console.log(
          "✅ Tauri command executed successfully - we're in Tauri context!"
        );
        return true;
      } catch (error) {
        const errorStr = error instanceof Error ? error.message : String(error);
        console.log(
          `🔄 Tauri command failed (${Date.now() - startTime}ms):`,
  ...
  console.log(
    "🔍 Final check - if invoke is available, we might still be in Tauri context"
  );
  ...
  try {
    ...
    if (!diagnostic.hasWindow) {
      ...
      diagnostic.suggestions.push(
        "Ensure code is running in browser environment"
      );
      return diagnostic;
    }
    ...
    if (!diagnostic.hasInvokeFunction) {
      ...
      diagnostic.suggestions.push(
        "Check if @tauri-apps/api/core is properly imported"
      );
    }

    // NEW: More lenient context check - if invoke is available, we're likely in Tauri
    // Even if __TAURI__ global isn't ready yet
    diagnostic.isInTauriContext =
      diagnostic.hasWindow && diagnostic.hasInvokeFunction;

    // Add warnings but don't fail if __TAURI__ global is missing
    if (!diagnostic.hasTauriGlobal && diagnostic.hasInvokeFunction) {
      console.warn(
        "⚠️ __TAURI__ global not available yet, but invoke function is present"
      );
      diagnostic.suggestions.push(
        "Tauri may still be initializing - this is often normal"
      );
    }
    ...
    if (!diagnostic.isInTauriContext) {
      if (!diagnostic.hasInvokeFunction) {
        diagnostic.errors.push(
          "Neither invoke function nor __TAURI__ global are available"
        );
        diagnostic.suggestions.push(
          "Run with 'npm run tauri dev' instead of 'npm run dev'"
        );
        diagnostic.suggestions.push(
          "Ensure Tauri dependencies are properly installed"
        );
      }
    }
  } catch (error) {
